...
export class BlobContainers {
  ...
  /**
   * This operation migrates a blob container from container level WORM to version level WORM enabled
   * container. Prerequisites require a container level immutability policy either in locked or
   * unlocked state, Account level versioning must be enabled and there should be no Legal holds.
   * @param resourceGroupName The name of the resource group within the user's subscription. The name
   * is case insensitive.
   * @param accountName The name of the storage account within the specified resource group. Storage
   * account names must be between 3 and 24 characters in length and use numbers and lower-case
   ...
   * @returns Promise<msRest.RestResponse>
   */
  versionLevelWormMethod(resourceGroupName: string, accountName: string, containerName: string, options?: msRest.RequestOptionsBase): Promise<msRest.RestResponse> {
    return this.beginVersionLevelWormMethod(resourceGroupName,accountName,containerName,options)
   ...
   */
  beginVersionLevelWormMethod(resourceGroupName: string, accountName: string, containerName: string, options?: msRest.RequestOptionsBase): Promise<msRestAzure.LROPoller> {
    return this.client.sendLRORequest(
      ...
      beginVersionLevelWormMethodOperationSpec,
      options);
  }
...
const beginVersionLevelWormMethodOperationSpec: msRest.OperationSpec = {
  httpMethod: "PUT",
